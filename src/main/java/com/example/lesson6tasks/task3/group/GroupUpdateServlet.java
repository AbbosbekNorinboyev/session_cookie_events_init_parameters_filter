...
@WebServlet(
        name = "com.example.lesson6tasks.task3.group.GroupUpdateServlet",
        value = "/update/group/*"
)
public class GroupUpdateServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        String pathInfo = req.getPathInfo();
        int id = Integer.parseInt(pathInfo.substring(1));
        Integer databaseUserId = LoginServlet.databaseUserId;
        try {
            DriverManager.registerDriver(new Driver());
            Connection connection = DriverManager.getConnection("jdbc:postgresql://localhost:5432/lesson6_task?currentSchema=public",
                    "postgres",
                    "2210");
            var preparedStatement = connection.prepareStatement("update groups g set group_name = ?, count = ?, created_by = ? where g.id = ?;");
            preparedStatement.setString(1, name);
            preparedStatement.setInt(2, count);
            preparedStatement.setInt(3, databaseUserId);
